...
class Correlation:
    def __init__(self):
        ...
        self.outputfile_cc = '../dataset/corr_data_cc.txt'
        self.outputfile_loy = '../dataset/corr_data_loy.txt'
    ...
    def correlation(self, stayEvent, consumeEvent, card):
        """求时空相似度"""
        if card == 'cc':
            return self.timeCorr1(stayEvent, consumeEvent, self.sigma1) * self.spaceCorr(stayEvent, consumeEvent, self.sigma2)
        else:                 # loyalty card
    ...
    def AveCorrelation_loyalty(self):
        """求loyalty card相似度矩阵"""
        id = 0
        for stayEvents in self.stay_data["stay_periods"]:
            res_row = []
...
if __name__ == '__main__':
    ...
    corr.AveCorrelation_loyalty()
    corr.saveData(corr.Correlation_loy, corr.outputfile_loy)
    print(corr.store)
